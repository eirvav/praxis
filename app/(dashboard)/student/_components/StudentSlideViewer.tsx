...
// Need to copy these interfaces since they are imported from SlideEditor in the original component
interface Slide {
  id: string;
  module_id: string;
  position: number;
  slide_type: 'text' | 'video' | 'quiz' | 'student_response' | 'slider' | 'context';
  config: TextSlideConfig | VideoSlideConfig | QuizSlideConfig | StudentResponseSlideConfig | SliderSlideConfig | ContextSlideConfig;
  created_at: string;
}
...
interface StudentResponseSlideConfig {
...
interface SliderSlideConfig {
  type: 'slider';
  sliders: SliderConfig[];
  description?: string;
}

interface ContextSlideConfig {
  type: 'context';
  content: string;
}
...
export default function StudentSlideViewer({ moduleId, estimatedDuration }: SlideViewerProps) {
  ...
  useEffect(() => {
    async function loadSlides() {
      ...
      try {
        ...
        console.log('[StudentSlideViewer] Loaded slides from database:', data);
        setSlides(data || []);
      } catch (err) {
  ...
  const renderCurrentSlide = () => {
    ...
    console.log('[StudentSlideViewer] Rendering slide:', currentSlide);
    ...
    switch (currentSlide.slide_type) {
      ...
      case 'video':
        ...
        console.log('[StudentSlideViewer] Video slide config:', currentSlide.config);
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="pt-4">
              <div className="space-y-8">
                {currentSlide.config.sliders.map((slider: SliderConfig) => {
